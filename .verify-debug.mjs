...
page.on("pageerror", (err) => logs.push(`ERROR: ${err}`));
...
for (let i = 0; i < 10; i++) {
  const nextX = box.x + Math.max(0.05, Math.min(0.95, cursorX / box.width - 0.06)) * box.width;
  ...
  const label = await findLabelCenter(page, "The Fuel Stop");
  if (label && Math.abs(label.x - (box.x + box.width / 2)) < 40) break;
}

await page.screenshot({ path: `${shotDir}/debug-aimed.png` });
...
await page.screenshot({ path: `${shotDir}/debug-final.png` });
console.log("Room final:", await page.locator("h2").first().textContent());
